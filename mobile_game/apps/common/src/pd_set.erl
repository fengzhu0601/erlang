...
new_from_list(Key, List) ->
    case erlang:get(Key) of
        undefined ->
            erlang:put(Key, sets:from_list(List))
    end,
    ok.
...
add_element_rb(Key, Element) ->
    case is_element(Key, Element) of
        true ->
            false;
        false ->
            NewS = sets:add_element(Element, erlang:get(Key)),
            erlang:put(Key, NewS),
            true
    end.

%% @doc same as add_element_rb but return elements
-spec add_element_rs(Key::term(), Element::term()) -> set:set().
...
del_element_rb(Key, Element) ->
    case is_element(Key, Element) of
        false ->
            false;
        true ->
            NewS = sets:del_element(Element, erlang:get(Key)),
            erlang:put(Key, NewS),
            true
    end.
...
-spec del_element_rs(Key::term(), Element::term()) -> set:set().
...
-spec fold(Key::term(), Fun, Acc0) -> Acc1 when
      ...
      AccOut :: T.
fold(Key, Fun, Acc0) ->
    sets:fold(Fun, Acc0, erlang:get(Key)).
...
new_test() ->
    ...
    ?assertEqual(sets:new(), erlang:get(Key)),
...
new_from_list_test() ->
    ...
    ?assertEqual(sets:from_list([1,2,3,4]), erlang:get(Key)),
    ?assertNotEqual(sets:from_list([1,2,3,4,5]), erlang:get(Key)),
    ?assertError({case_clause, _}, new(Key)),
    erase(Key),
    ok.
...
erase_test() ->
    ...
    ?assertEqual(sets:to_list(sets:from_list([1,2,3,4])), erase(k2)),
    ok.
...
is_empty_test() ->
    ?assertError({badrecord, set}, is_empty(k1)),
    new(k1),
    io:format("ke1:~p~n", [erlang:get(k1)]),
    ?assert(is_empty(k1)),
    erase(k1),
    ok.
...
is_element_test() ->
    ?assertError({badrecord, set}, is_element(k1, 1)),
...
clear_test() ->
    ...
    ?assertEqual(ok, new_from_list(k2, lists:seq(1,4))),
    ?assertEqual(sets:to_list(sets:from_list(lists:seq(1,4))), clear(k2)),
    ?assert(is_empty(k2)),
    ?assert(is_exist(k2)),
...
add_element_rb_test() ->
    ...
    ?assertEqual(sets:from_list(lists:seq(1,4)), erlang:get(k1)),
...
add_element_rs_test() ->
    new(k1),
    ?assertEqual(sets:from_list([1]), add_element_rs(k1, 1)),
    ?assertEqual(sets:from_list([1,2]), add_element_rs(k1, 2)),
    ?assertEqual(sets:from_list([1,2,3]), add_element_rs(k1, 3)),
    ?assertEqual(sets:from_list([1,2,3,4]), add_element_rs(k1, 4)),

    ?assertEqual(sets:from_list([1,2,3]), del_element_rs(k1, 4)),
    ?assertEqual(sets:from_list([1,2]), del_element_rs(k1, 3)),
    ?assertEqual(sets:from_list([1]), del_element_rs(k1, 2)),
    ?assertEqual(sets:from_list([]), del_element_rs(k1, 1)),
    ?assert(is_empty(k1)),
...
-define(T, 100000).
                                                ...
                                                %create 1000000 times star:{1377,857419,310888}
                                                %create 1000000 times over:{1377,857420,412374}
                                                %100W/s
create() ->
    S = erlang:now(),
                                                %create(1, ?T),
