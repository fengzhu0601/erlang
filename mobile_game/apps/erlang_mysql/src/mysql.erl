...
%% == Modified Log==
%%
%% Modified: 9/12/2006 by Yariv Sadan<br/>
...
%%
%% Modified: 9/23/2006<br/>
%% Rewrote the transaction handling code to
...
%%
%%   `start_link(PoolId, Host, User, Password, Database)'<br/>
%%   `start_link(PoolId, Host, Port, User, Password, Database)'<br/>
%%   `start_link(PoolId, Host, User, Password, Database, LogFun)'<br/>
...
%% query :
%%
%%   `fetch(PoolId, "select * from hello") -> Result'<br/>
...
%%          and AllRows   = list() of list() representing records'<br/>
%%     - on update:<br/>
%%          `Affected  = mysql:get_result_affected_rows(MysqlRes)
%%         with Affected  = integer()'<br/>
%%     - on error:<br/>
...
-include("../../apps/common/include/com_log.hrl").

-define(MYSQL_CALL_TIMEOUT_3S,3000).    %普通的call超时3s
...
start1(PoolId, Host, Port, User, Password, Database, LogFun, Encoding,
       StartFunc) ->
    Re = crypto:start(),
    %io:format("start1 crypto===---------------------------:~p~n",[Re]),
...
connect(PoolId, Host, Port, User, Password, Database, Encoding, Reconnect,
    ...
    try
        gen_server:call(?SERVER, get_logfun,?MYSQL_CALL_TIMEOUT_3S)
    of
        ValueRet ->
            case ValueRet of
                {ok, LogFun} ->
                    case Fun(Host, Port1, User, Password, Database, LogFun, Encoding, PoolId) of
                        {ok, ConnPid} ->
                            ...
                            try
                                case gen_server:call(?SERVER, {add_conn, Conn},?MYSQL_CALL_TIMEOUT_3S) of
                                    ok ->
                                        {ok, ConnPid};
                                    Res ->
                                        Res
                ...
                _ ->
                    ?ERROR_LOG("no get log fun"),
                    error
            end
    catch
        ELog:WLog ->
            ?ERROR_LOG("ERROR ELog=~p WLog=~p",[ELog,WLog]),
            error
    end.
...
fetch_cast(PoolId, Query) ->
    fetch_cast(PoolId, Query, undefined).

fetch_cast(PoolId, Query, _Timeout) ->
    case get(?STATE_VAR) of
        undefined ->
            cast_server({fetch, PoolId, Query});
        State ->
            mysql_conn:fetch_local(State, Query)
    end.
...
get_prepared(Name, Version) ->
    try
        gen_server:call(?SERVER, {get_prepared, Name, Version},?MYSQL_CALL_TIMEOUT_3S)
    catch
        E:W ->
            ?ERROR_LOG("error E=~p W=~p",[E,W]),
            error
...
%% ignored.
%%
%% @spec execute(PoolId::atom(), Name::atom()) -> mysql_result()
execute(PoolId, Name) when is_atom(PoolId), is_atom(Name) ->
    execute(PoolId, Name, []).

%% @spec execute(PoolId::atom(), Name::atom(), [term()] |
%%   integer()) -> mysql_result()
...
init([PoolId, Host, Port, User, Password, Database, LogFun, Encoding]) ->
	...
	{ok, ConnPid} ->
		io:format("mysql init ------------------------:~p~n",[ConnPid]),
	...
	{error, Reason} ->
		io:format("mysql init Reason------------------------------:~p~n",[Reason]),
	    ?Log(LogFun1, error,
...
handle_cast({fetch, PoolId, Query},State) ->
    fetch_queries(PoolId, nil, State, [Query]),
    {noreply, State};
...
call_server(Msg, Timeout) ->
    try
        if
            Timeout == undefined ->
                gen_server:call(?SERVER, Msg,?MYSQL_CALL_TIMEOUT_3S);
            true ->
        ...
        E:W ->
        	%% need open
            %?ERROR_LOG("call time out E=~p W=~p",[E,W]),
            error
    end.
